...
mondters = sprite.Group()
for i in range(1, 6):
    mondter = Enemy(img_enemy, randint(80, win_width-80), -60, 80, 50, randint(1,5))
    mondters.add(monster)
font.init()
font1 = font.Font(None, 80)
font2 = font.Font(None, 36)
win = font1.render('You win', True, (255, 255, 255))
lose = font1.render('You lose', True)
...
while run:
    ...
    for e in event.get():
        if e.type == QUIT:
            ...
            if e.key == K_SPACE:
                if mun_fire < max_fire and real_time == False:
                    num_fire += 1
                    fire_sound.play()
                    ship.fire()
    ...
    if not finish:
        ...
        collides = sprite.groupcollide(mondters, bullets, bullets, True, True)

        for c in collides:
            score += 1
            mondter = Enemy(img_enemy, randint(80, win_width-80), -40, 80, 50, randint(1,5))
        mondters.add(mondter)
    ...
    if sprite.spritecollide(ship, monsters, False):
        sprite.spritecollide(ship, mondters, True)
        life -= 1
